...
import { transformSkill, TransformOptions } from './transformer.js';
...
function updateAgentsMd(newXml: string): void {
    let content = '';
...
async function cmdTransform(args: string[]): Promise<void> {
...
async function cmdInstall(url: string): Promise<void> {
    if (!url) {
