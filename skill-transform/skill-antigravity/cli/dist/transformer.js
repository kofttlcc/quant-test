...
export async function transformSkill(skillPath, options = {}) {
    ...
    try {
        ...
        const finalContent = matter.stringify(header + '\n\n' + transformedContent, newFrontmatter);
        // 5. 輸出
 ...
 * 轉換 Frontmatter
 */
function transformFrontmatter(original, skillName) {
    return {
 ...
 * 轉換內容
 */
async function transformContent(content, options) {
    let result = content;
    // 翻譯標題 (## Heading)
 ...
 * 翻譯標題
 */
function translateHeadings(content) {
    let result = content;
 ...
 * 翻譯常見術語
 */
function translateTerms(content) {
    let result = content;
 ...
 * 生成 Antigravity 標準頭部
 */
function generateAntigravityHeader(skillName, originalFrontmatter) {
 ...
 * 插入使用情境區塊
 */
function insertUsageSection(content) {
    const usageSection = `
## 使用情境
 ...
 * 添加專案整合說明
 */
function appendProjectIntegration(content) {
    const integration = `
